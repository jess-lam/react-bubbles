...
Tokens are strings of cryptic texts that are used for servers to issue to the client and read to make decisions for data transfer based on the client's permission.

- [ ] What steps can you take in your web apps to keep your data secure?

Using protected routes can ensure that the client has the correct credentials to view confidential information. Creating and issuing a unique token will also make the web app more secure by only issuing the server request when the token is present.

- [ ] Describe how web servers work.

A web server stores copies of data. When a client uses an IP address or URL to send a request to the server, the server processes in the request and sends the requested data back to the client.
...
- [ ] **RUN** `yarn start or npm start` to get your API up and running on `http://localhost:5005`. This is the **URL** you're going to need to use within your React app in order to make AJAX requests for data.
